...
def avaliar():
    ...
    if not os.path.exists(caminho_json):
        logger.error(f"O arquivo {caminho_json} não foi encontrado!")
        sys.exit(1)
    ...
    logger.info(f"Iniciando avaliação de {len(questoes)} questões técnicas...\n")
    ...
    for i, item in enumerate(questoes, 1):
        ...
        try:
            ...
            if "Não encontrei informações suficientes" in resposta_agente or "[ERRO]" in resposta_agente:
                logger.warning("FALHA (O agente não localizou a informação no banco vetorial)")
            else:
                prompt_juiz = f"""
                ...
                if "CORRETO" in julgamento and "INCORRETO" not in julgamento:
                    sucessos += 1
                    logger.info("SUCESSO (Resposta validada pelo Juiz!)")
                else:
                    logger.warning("FALHA (A resposta foi dada, mas está errada segundo o gabarito)")
        
        except Exception as e:
            logger.error(f"Erro ao processar questão {i}: {e}")
